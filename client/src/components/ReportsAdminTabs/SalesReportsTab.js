...
const SalesReportsTab = ({ allOrders, products }) => {
    ...
    const preparePieData = (orders) => {
        const { cogs, grossMargin, shippingCosts } =
            accumulateMarginAndCogs(orders)
        const pieData = [
            { name: 'Shipping Costs', value: shippingCosts },
    ...
    const isTargetDate = (targetDate, testDate) => {
        return (
            testDate.toDate().getDate() === targetDate.getDate() &&
    ...
    const getDailyTotals = (orders) => {
        ...
        for (let i = 0; i < today.getDate(); i++) {
            ...
            const dailyOrders = orders.filter((order) =>
                isTargetDate(targetDate, order.orderDate)
            )
            let dailyCOGS = 0
            ...
            dailyTotals.push({
                ...
                shippingCosts: dailyShippingCost,
                shippingFees: dailyShippingFee,
            })
        }
        return dailyTotals
    }
    ...
    const getMonthlyTotals = (orders) => {
        ...
        for (let i = 0; i <= month; i++) {
            ...
            const monthlyOrders = orders.filter((order) =>
                isSameMonth(targetDate, order.orderDate)
            )
            let monthlyCOGS = 0
            ...
            monthlyTotals.push({
                ...
                shippingFees: monthlyShippingFee,
                shippingCosts: monthlyShippingCost,
            })
        }
        ...
        <div>
            ...
            <Form.Group controlId="selectView">
                ...
                <Form.Control as="select" onChange={(e) => onChange(e)}>
                    <option eventKey="month" value="month">
                        This Month
                    </option>
                    <option eventKey="year" value="year">
                        This Year
                    </option>
                </Form.Control>
            </Form.Group>
