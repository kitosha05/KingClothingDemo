...
const BulkEditInventory = ({products, collections, fetchProductsStart})=>{
    ...
    const onSave = ({id, newInventory, newCOGS}) => {
        ...
        const saveRes = updateProduct({productId, product})
        onCancel()
    }
    ...
    return(
        <div>
            ...
            <Form.Group controlId="selectCollection">
                    ...
                    <Form.Control as="select" name="selectCollection" onChange={(e)=>onSelectCollection(e)} 
                          value={selectedCollection}
                          required>
                         {
                            collections ? renderCollectionTitles(collections) : null
                        }    
                 </Form.Control>
                </Form.Group>
            </Form>
        ...
        <TableBody>
                        { !products || !selectedCollection ? "" : (
                             products.filter(product=>product.collection===selectedCollection).map(product=>{
                                 return(
                                     <TableRow key={product.id}>
                                         ...
                                         <TableCell>
                                         {
                                                          inEditMode.status && inEditMode.rowKey === product.id ? (
                                                              <input value={currentInventory}
                                                                     onChange={(event) => changeInventory(event)}
                                                              />
                                                          ) : (
                                                          ...
                                                          inEditMode.status && inEditMode.rowKey === product.id ? (
                                                              <React.Fragment>
                                                                  <button
                                                                      ...
                                                                      onClick={() => onSave({id: product.id, newInventory: currentInventory, newCOGS: currentCOGS})}
                                                                  >
                                                                      Save
                                                                  </button>
                                                              ...
                                                              <button
                                                                  ...
                                                                  onClick={() => onEdit({id: product.id, currentInventory: product.inventory ? product.inventory : '', currentCOGS: product.cogs ? product.cogs : ""})}
                                                              >
                                                                  Edit
                                                              </button>
                                                          )
