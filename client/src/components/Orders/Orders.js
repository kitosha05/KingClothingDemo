...
const Orders = ({ allOrders, fetchOrdersStart }) => {
    ...
    const sendReadyForPickUpEmail = (order) => {
        window.fetch('/email/ready-for-pickup', {
            method: 'POST',
            headers: {
    ...
    const sendShippingConfirmation = (order) => {
        window.fetch('/email/shipping-confirmation', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
    ...
    const sendPickedUpConfirmation = (order) => {
        window.fetch('/email/was-picked-up', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
                ...
                <TableBody>
                    {rows.map((row) => (
                        <TableRow key={row.id}>
                            ...
                            <TableCell>
                                <Link to={`/admin/orders/${row.id}`}>VIEW</Link>
                            </TableCell>
                            <CustomModal
                                ...
                                orderId={row.id}
                                handleConfirm={
                                    row.status === 'Prepare For Pickup'
                                        ? readyForPickUp
                                        : row.status === 'Prepare For Shipping'
                                        ? readyForShipping
                                        : wasPickedUp
                                }
                                show={show === row.id}
