    ...
    getInventoryValueByCollection(products) {
        ...
        if (products) {
            products.map((product) => {
                ...
                if (collection === 'Hats')
                    hatsInventory += this.getProductInventoryValue(product)
                if (collection === 'Jackets')
                    jacketsInventory += this.getProductInventoryValue(product)
                if (collection === 'Womens')
                    womensInventory += this.getProductInventoryValue(product)
                if (collection === 'Mens')
                    mensInventory += this.getProductInventoryValue(product)
                if (collection === 'Sneakers')
                    sneakersInventory += this.getProductInventoryValue(product)
            })
        }
        const inventoryData = [
            {
            ...
            {
                Collection: 'Jackets',
                inventoryValue: jacketsInventory,
            },
            {
                Collection: 'Sneakers',
        ...
        inventoryData.map(
            (collection) => (totalInventory += collection.inventoryValue)
        )
